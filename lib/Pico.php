...
class Pico
{
    ...
    /**
     * Loads config.yaml and any other *.yml from Pico::$configDir
     *
     * After loading {@path "config/config.yaml"}, Pico proceeds with any other
    ...
    protected function loadConfig()
    {
        ...
        // load main config file (config/config.yaml)
        $this->config = is_array($this->config) ? $this->config : array();
        if (is_file($this->getConfigDir() . 'config.yaml')) {
            $this->config += $loadConfigClosure($this->getConfigDir() . 'config.yaml');
        }

        // merge $config of config/*.yml files
        $configFiles = $this->getFilesGlob($this->getConfigDir() . '*.yml');
        foreach ($configFiles as $configFile) {
            if ($configFile !== $this->getConfigDir() . 'config.yaml') {
     ...
     * {@path "config/config.yaml"} or changing some of its variables before
     ...
     * overwritten by {@path "config/config.yaml"}.
     *
     * @see Pico::loadConfig()
     * @see Pico::getConfig()
     *
